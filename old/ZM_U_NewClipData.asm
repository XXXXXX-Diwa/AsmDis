.gba
.open "zm.gba","New Clip.gba",0x8000000

.definelabel SpikeClipNum,0x90
.definelabel SpikeDamage,11
.definelabel KnockbackVelocityX,0x12
...
; check invincibility
	...
	add     r3,1           ;Y坐标加1,也就是下部边界,向下1h
	sub     r4,1           ;左边边界减1,也就是加宽1h
	add     r5,1           ;右边边界加1,也就是加宽1h
	sub     r6,1           ;上部边界减1,也就是加高1h
...
@@CheckLeftSide:
	mov     r1,r2
	mul     r1,r6          ;房间宽度乘以上部边界格数
	add     r1,r1,r4       ;加上左部边界格数乘以2
	lsl     r1,r1,1
	ldr     r0,=DecompClipdata
	add     r1,r0,r1       ;加上数据偏移
	ldrh    r0,[r1]
	cmp     r0,SpikeClipNum   ;比较是否是刺砖
	beq     @@TouchingSpikeRight ;如果是的话,则判断碰到右边的刺砖  
	cmp     r4,r5          ; check if horizontal blocks are same  左部边界格数和右部边界格数比较是否相等
	beq     @@Loop
; check block to right
	add     r0,r1,2        ;左部边界的格数向右1格
	ldrh    r0,[r0]
	cmp     r0,SpikeClipNum   ;检查是否是瓷砖
