.gba
.open "zm.gba","smoothing.gba",0x8000000

.org 0x80075C4			;r0 = 30013F4,  r2 = 30013D4, r3 = 3001528 at hijack start
	ldr r1,=8f00001h	;make r1 freespace, if used on vanilla rom the current offset is okay
...
.org 0x8008464
    nop	
.org 0x8f00000		;Freespace
	push r14	
	mov r1,r2
	ldrb r0,[r2,1h]   ;读取当前是否在空中
	cmp r0,2h
	bne @@CheckOnland
	mov r1,r2
	sub r1,58h
...
@@LandingCheck:	
	...
	ldrb r0,[r1]
	cmp r0,0FDh	;seems to only be true if landing from a jump 好像只有跳跃落地才是正确的，然而没找到过
	bne @@MorphJump
